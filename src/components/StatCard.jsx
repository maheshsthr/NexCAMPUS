export default function StatCard({ icon: Icon, value, label, change, barClass = 'bg-[#6C5CE7]', iconClass = 'bg-[#6C5CE7]' }) {
  return (
    <div className="bg-white dark:bg-[#1E293B] rounded-2xl p-5 flex items-center gap-4 relative overflow-hidden shadow-sm border border-gray-200 dark:border-white/10 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg min-w-0">
      ...
      {change && <span className="ml-auto text-[10px] sm:text-xs font-semibold text-green-600 bg-green-50 px-1.5 sm:px-2.5 py-1 rounded-full whitespace-nowrap hidden sm:inline">{change}</span>}
    </div>
  )
}
